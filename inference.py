...
# Modified for customizable checkpoint and step parameters
...
def get_parser(**parser_kwargs):
    ...
    # New arguments for customizable checkpoint and step
    parser.add_argument(
            "--ckpt_path",
            type=str,
            default="",
            help="Custom checkpoint path. If not provided, will use default path based on task and version."
            )
    parser.add_argument(
            "--step",
            type=int,
            default=None,
            help="Custom step value. If not provided, will use default step based on task and version."
            )
...
def get_configs(args):
    ...
    # Determine step value: custom > default
    if args.step is not None:
        step = args.step
    else:
        if args.task == 'realsr':
            step = _STEP[args.version]
        else:
            step = _STEP[args.task]
    ...
    if args.task == 'realsr':
        ...
        if args.ckpt_path:
            ...
            ckpt_path = ckpt_dir / f'resshift_{args.task}x{args.scale}_s{step}_{args.version}.pth'
        ...
        if args.ckpt_path:
            ...
            ckpt_path = ckpt_dir / f'resshift_{args.task}x{args.scale}_s{step}.pth'
        vqgan_url = _LINK['vqgan']
        vqgan_path = ckpt_dir / f'autoencoder_vq_f4.pth'
        ...
        if args.ckpt_path:
            ...
            ckpt_path = ckpt_dir / f'resshift_{args.task}_s{step}.pth'
        vqgan_url = _LINK['vqgan']
        vqgan_path = ckpt_dir / f'autoencoder_vq_f4.pth'
    ...
    # prepare the checkpoint (only download if using default path and file doesn't exist)
